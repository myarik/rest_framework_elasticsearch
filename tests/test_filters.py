...
class TestElasticOrderingFilter:
    ...
    def test_get_valid_fields_without_es_ordering_fields(self):
        ...
        self.backend.get_default_valid_fields = lambda q, v, c: default_valid_fields
        result = self.backend.get_valid_fields([], view)
        assert result == default_valid_fields
...
class TestElasticFieldsFilter:
    ...
    def test_filter_search(self, search, es_filter_fields, query_params, expected):
        view = self.create_view(es_filter_fields)
        request = rf.get('/test/')
...
class TestElasticFieldsRangeFilter:
    ...
    @pytest.mark.parametrize('es_range_filter_fields, query_params, expected', [
        (
            [ESFieldFilter('score')],
            {'from_score': '500'},
            [6, 7, 8, 10, 11]
    ...
    def test_filter_search(self, search, es_range_filter_fields,
...
class TestElasticSearchFilter:
    ...
    def test_get_es_query(self):
        ...
        class CustomElasticSearchFilter(ElasticSearchFilter):
        ...
        s_query = "test"
        s_fields = "first_name"
        backend = CustomElasticSearchFilter()
